...
import { fetchAllJobs, fetchJob } from "@/lib/api";

// export async function generateStaticParams() {
	
// 	const jobs = await fetchAllJobs();
// 	if (!jobs || jobs.length === 0) {
// 		console.warn("No jobs found to pre-generate pages");
// 		return [];
// 	}
...
//   console.log(`Fetched job with ID: ${jobId}`, job);
...
export default async function JobDetailPage({ params }) {
  ...
  const res = await await fetch(`http://localhost:5000/api/job/${jobId}`);
  if (!res.ok) {
    notFound(); // Show 404 page
  }
